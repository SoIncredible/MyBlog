...
我们在实际的开发中往往会有各种意外情况出现导致当我们按照上面正常的操作还是会导致推送到远端的时候发生了冲突，这是难以避免的，而且发生冲突的原因千变万化，以我目前的能力很难给它们归因，我能够分享给你的是当我在遇到冲突的时候我是如何运用git提供的功能来解决冲突的
...
变基在我的git使用中几乎很少用到，我对变基的概念理解也不深刻，不过在此还是简单介绍一下我们在什么场景下会使用到变基，由于，那是因为我使用了SourceTree帮我处理了很多东西

变基在官方文档中的解释是： Reapply commits on top of another base tip，我的理解是，变基允许我们在当前分支上所做的提交移接到另外一个分支最新一次提交的后面

举一个例子帮助各位理解

在本地有一个master分支，在master分支的节点A处拉出了新的分支dev，你比较厉害，在master上开发功能A的时候同时在dev分支上开发功能B，这导致在master分支节点A之后你有进行了多次提交，同时你也在dev上进行了多次提交，当你完成了dev上的开发工作之后，你想将dev分支的代码合并到master分支上，你当然可以选择使用merge的方式将dev分支合并到master分支上，但如果说dev分支和master分支上开发的是两个完全不重叠的功能的话，那么这一次合并其实是一个无意义的合并，这时我们就可以使用变基的方式避免分支的产生，在dev分支上使用`git rebase master`再切换到master分支上，我们发现我们的master分支上最新几次的提交就变成dev分支上最新几次的提交。
...
git pull的含义相当于git fetch + git merge，这种操作会导致很多不必要的合并使得git的分支看起来十分地混乱

在git merge中会有两种结果产生，一种是快进合并，一种是非快进合并，一般情况下如果本地从某一个时刻拉取下远端仓库之后没有做任何修改，在远端仓库有了新的提交之后再次拉取的时候就会进行快进合并，从而产生真正的合并而如果本地进行了提交并且远端的分支状态也发生了更新，那么在拉取的时候就会发生非快进提交，也就会产生合并
...
git钩子是一个触发器，当你在进行一些操作的时候，比如你在commit或者push的时候

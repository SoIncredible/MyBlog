...
我认为使用Git对于我的开发工作的意义有两点：1.它帮助我轻松地实现版本管理；2.通过版本管理我可以将我的仓库上传到云端，实现多端同步开发。上传云端是一个很有必要的操作，本质是为了进行数据备份，如果本地的设备遭受到了物理上的损坏和不可还原修复，那么我们的工作将全部丢失，上传到云端也是为了保险起见。如果我们有自己的云服务器，我们完全可以把仓库部署在自己的云服务器上，然后实现多端同步开发。在自己的云服务器上搭建Git仓库这件事情不在本文的探讨范围内，但是未来某一天我或许会尝试这件事，先给自己记一个TODO。
...
我们创建仓库的一般做法，或者说基本的规范是，文件夹的根目录的名字应该和远端仓库的名字一致，并且尽量不要带有空格，如果名字需要有多个单词的话可以使用-号或者_下划线来表示单词的分割

比如现在我们要创建一个名字叫做StarryTown的文件夹，我们可以使用Starry-Town或者Starry_Town的格式来在本地创建这个文件夹

紧接着我们需要在云端创建和本地文件夹同名的仓库，我们不需要勾选创建README和gitignore文件的选项，我们会在第一次提交的时候自己将README和gitignore文件提交并且push到远端仓库，由于每次都要手动创建这两个文件比较麻烦，所以我们可以自己写一个终端命令的方式来自动化运行创建这两个文件并且向这两个文件中写入内容。具体操作方法在这篇博客中。
...
第一次提交很容易出问题，我曾经因为忘记创建gitignore文件，导致一些你不必要的文件被提交进去，虽然可以通过删除仓库重新再初始化然后添加gitignore的方式解决，但是这种方案看起来不够优雅从容，因此下面我给出了在第一次提交的时候存在错误并想要不通过删除仓库的方式撤回第一次提交的方案。虽然SourceTree是一个十分好用的可视化版本控制工具，但是它在面对上述情况的时候也是无能为力，我们只能通过终端输入git命令撤回我们的第一次提交。

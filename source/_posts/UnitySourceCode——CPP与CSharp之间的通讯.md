...
T dynamic_pptr_cast(U* ptr)
{
    ...
    T castedPtr = (T)(ptr);
    if (castedPtr && castedPtr->template IsDerivedFrom<Type>())
        return castedPtr;
    else
        return NULL;
}
...
template<typename T>
T* PPtrToObjectDontLoadNoThreadCheck(PPtr<T> pptr)
{
    if (pptr.GetInstanceID() == InstanceID_None)
        return NULL;

    Object* temp = Object::IDToPointerNoThreadCheck(pptr.GetInstanceID());

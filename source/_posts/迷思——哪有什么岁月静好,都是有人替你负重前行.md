...
比如你用List组织了一组Player, 在每一帧的时候
```C#
foreach(var player in Players){
    player.Update();
}
```
首先揭开第一重迷雾, 将foreach还原
```C#
...
```
第二重迷雾 如何MoveNext的? (如何控制流程的呢?) C#底层的CLR控制的
```C#
while已经很简洁明了了, 所以就先到此为止

```

第三重迷雾 MoveNext里面做了什么, 对于一个List来说

```C#

```

第四重迷雾Player.Update里面做了什么 定义了行为
```C#
void Update(){
    UpdateMovement();
    UpdateAnimations();

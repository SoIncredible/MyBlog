...
2023.04.29更新：今天是五一假期的第一天，时间过的好快，已经在公司实习将近两个月了，这段时间学到了不少的东西，也记录了不少的博客，但是整体上内容比较零散。这两个月的时间我学习的思路也一直在发生转变，今天我再回过头来看我这篇博客的时候发现很多的笔墨都花费在了细枝末节上，虽然搞懂了很多小的知识点，但这并不利于我对项目的理解，自己钻牛角尖的毛病要稍微注意注意了。
...
所以为什么我们调用父类的Prepare，它能够自动地去调用子类的Prepare方法？
...
在C#中，`where`关键字用于约束泛型类型参数。它允许您为泛型类型或方法指定额外的限制条件或特定接口。这种约束确保泛型类型参数具有所需的特性，例如实现特定接口、具有可用构造函数或符合某一基类。

以下举例了不同类型的约束以及其用法：

1. 基类约束：要求类型参数必须具有特定基类或者派生自特定基类

   ```C#
   public class GenericBaseClass<T> where T : MyBaseClass{
   	// ...
   }
   ```

2. 接口约束：要求类型参数必须实现一个或多个接口

   ```C#
   public interface MyInterface{}
   public class GenericWithInterface<T> where T : MyInterface{
   	// ...
   }
   ```

   对于多个接口约束，将它们使用逗号进行分割：

   ```C#
   public interface MyInterface1{}
   public interface MyInterface2{}
   
   public class GenericWithMultipleInterfaces<T>
   	where T : MyInterface1, MyInterface2{
   		// ...
   	}
   ```

3. 引用类型约束`class`：要求类型参数必须是引用类型

   ```C#
   public class ReferenceTypeGeneric<T> where T : class{
   	// ...
   }
   ```

4. 值类型约束`struct`：要求类型参数必须是非空的值类型（不能是可空值类型）

   ```C#
   public class ValueTypeGeneric<T> where T : struct{
   	// ...
   }
   ```

5. 无参构造函数`new()`约束：要求类型参数具有无参公共构造函数。请注意，`new()`约束必须放在约束列表的末尾

   ```C#
   public class NewableGeneric<T> where T : new(){
   	// ...
   }
   ```
...
6. 为泛型方法参数指定约束：
   ...
   ```C#
   public void GenericMethod<T>(T param) where T : IComparable<T>{
    // ...
   }
   ```

   可以根据需要在这些类型约束中组合使用多详细约束

   ```C#
   public class ComplexGeneric<T> where T : MyBaseClass, MyInterface1, Myinterface2, new(){
   	// ...
   }
   ```

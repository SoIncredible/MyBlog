...
拆箱不是直接将装箱过程倒过来, 拆箱过程本身会复制任何类型, 但是拆箱过后往往跟随着一次字段的复制, 我们需要在栈上复制一个堆中的字段, 并在栈上对改字段的值进行修改. 

## 反复地拆装箱

反复地拆装箱会产生额外的IL代码, 增加代码体积. 更大的问题是, 额外的装箱步骤会在托管堆中分配额外的对象, 将来必须对其进行垃圾回收. 拆装箱的滥用会严重影响程序的性能和内存消耗.

如此一来，obj指向了堆上值为11的对象。堆上值为10的对象就等待GC的回收，从上面这个角度可以稍微理解，为什么装箱拆箱会引起GC。

## 装箱拆箱的开销来自哪里

所以，值类型装箱后是不能改变它的值的，装箱后的值具有恒定性（Immutable）的特点，如果想给引用类型的变量赋予新的值，那就需要再堆上另开辟新的内存空间，一旦一个值类型被装箱，它的值就没有办法改变了。

在实际写代码的过程中,我们肯定不会闲得声明一个object类型的字段,却用一个值类型的数据给它赋值,比如上面的`object obj = 10`. 实际开发中, 拆装箱更可能发生在开发者无意识地将值类型参数传入参数为引用类型的接口中. 我们为了让我们设计的接口的兼容性尽可能的高, 很有可能把接口传入的参数类型设计为object类型, 请注意, 装箱和拆箱是两个阶段, 当我们调用该接口时, 将一个值类型作为参数传入, 仅仅导致了装箱操作. 在这个接口内部, 你可能只是将传进来的参数作为log输出或者别的, 那么就不会发生拆箱操作, 只有当你, 不过这种情况在实际开发中也是比较少见的, 因为大部分开发者想都不想只会把class传来传去, 因此根本就不存在拆装箱的操作, 因此笔者认为, 拆装箱问题在实际开发中很少遇见, 但这并不代表着实际开发中就可以滥用object了, 主要原因是object类型的使用通常伴随着类型检查与类型转换, 这就是下一小节的内容了.
...
因此, 如果开发者能够肯定 类型转换必定成功 且没有拆装箱 那么类型转换的开销 只有 判断能否类型转换逻辑执行所需要的开销
有, 不同情况下的开销还不太一样,
一般来讲, 类型的转换过程如下: 代码中执行类型转换语句, IL中做一个逻辑判断, 是否能够类型转换, 如果能则转换成功, 如果不能则抛出异常, 因此, 类型转换造成的额外开销来自于做判断能否转换的判断, 不过这个开销不是很高的

如果类型转换中设计值类型, 那么就有可能面临拆装箱操作, 开销就是判断能否转换+拆箱/装箱(注意这里的用词,类型转换要么是装箱,要么是拆箱, 装箱拆箱不一定非要成对出现的)造成的开销.

如果是泛型约束的话, 那情况就不太一样了, 运行时CLR不会对传入的参数做任何类型检查, CLR完全信任该泛型的类型, 或者你可以理解为, 使用泛型实现的类型转换, 是在编译时就搞定了, 因此运行时不需要做 或者说运行时根本就没有类型转换这一步. 因此是最高效的一种类型转换.
...
public void Func(object param){
    ...
    if(param is ClassA classAInst){

    }

    var p = param as ClassA;

}

```

> 注意 使用`is`操作符和`.GetType()`接口都可以做类型检查, 但是两者的开销是不同的, 按照GPT的说法, `is`操作符的性能开销要小于`.GetType()`的开销, 原因是
> 使用`is`操作符, C#编译器生成直接的类型检查指令(isinst IL指令), 无需触发完整的类型元数据加载.
> is操作符会利用类型继承关系进行快速判断,无需获取完整的Type对象
> 值类型优化:如果是值类型, is会避免装箱, 前提是obj是值类型且类型兼容
>
> 使用`.GetType()`时
> 会触发完整类型元数据加载,需要访问对象的Type对象,涉及元数据查询
> 精确类型比较: GetTypE()返回的是对象的实际运行时类型, 与typeof(MyType)比较时,只有当obj的运行时类型完全等于MyType时才返回true
> 许方法调用开销 `.GetType()`是虚方法调用, 需经过虚方法表查找.
>
> 当然, .GetType也有其用武之处, 可以用来做精确查找, 比如你希望找到基类,那么你传入一个子类实例进去后,使用is操作符会返回true,使用GetType方式会返回false, GetType要求类型必须是完全匹配

按照GPT的说法, 泛型是一种能够规避类型转换、且保证类型安全的双赢方法, 泛型在运行时做类型转换时,是不需要判断类型安全的, 直接转换, 不需要开销, 因为泛型在编译时就已经确保类型转换是安全的了.

因此, 滥用object作为接口的参数并不是一种优雅的方式, 更优雅的方式是使用多态或者泛型, 因为这两种方式不需要做类型检查. 但是两种方式都有各自的缺点, 使用多态你可以在基类中定义新的行为, 所有的派生类中就都具有了新的行为, 但是终究要面临庞大的继承体系, 并不好管理. 使用泛型则可以让传入的参数更加灵活, 但是它们的行为又很难规整到一起, 

这时笔者想到了一种结合的办法, 利用泛型+接口来实现,用泛型保证传入参数的灵活性, 用接口约束传入参数的行为. 

# 值类型中持有引用类型 该引用类型的生命周期如何? 闭包对其的影响如何?

使用函数闭包来验证一下生命周期

# 引用类型中持有值类型 该值类型的生命周期如何? 通过闭包能够延长该引用类型的生命周期, 还能正确访问该值类型吗?

使用函数闭包来验证一下生命周期

# 基于C#的这种特性, 很适合用在UIMgr 打开页面的时候需要向页面里面传入特定的数据结构时 或者状态机里面的黑板数据 状态的EnterState里面需要传入特定类型的数据, 是否可以利用泛型给每一个状态定义一个特定的类型?

通过下面这种方式,规避UIShowData的继承
...
namespace UI
{
    ...
    public class UIMgr
    {
        ...
        public void ShowWindow<T>(EUIType euiType, T args)
        {
            var ui = new UIA();
            ui.OnShow(args);
        }
...
namespace UI
{
    ...
    public class BaseUI
    {
        public void OnShow<TUIShowData>(TUIShowData args)
        {
            
        }
    }
    
    public struct UIAShowData
    {
        
    }
    
    public class UIA : BaseUI
    {
...
新的问题, 上面这样写和下面这样写的区别是什么?
```
public class BaseUI<TUIShowData>
{
    public void OnShow(TUIShowData args)
    {
...
```
如果按照下面这么写 在实例化UIA的时候, 泛型就能确定了, 泛型的作用是, 让使用的类型尽可能地在调用的时候确定, 将泛型写在类后面值得是在实例化的时候就确定了类型, 而写在一个方法后面指的是在实例化该类的时候还没有确定该泛型的具体类 等调用该方法的时候才能确定泛型的具体类
```
public void ShowWindow<T>(EUIType euiType, T args)
{
    // 这里已经可以确定UI的类型了
...
```
下面这个方法中, 虽然两个方法上都标记了TUIShowData泛型,但是两个方法调用时具体使用什么类型是彼此无关的
```
public class BaseUI
{
    // 构造方法不能使用泛型
    ...
    public void OtherFunc<TUIShowData>(TUIShowData data)
    {
